...
# 6.17.20 updated covidence data
...
file <- 'review_47118_extracted_data_csv_20200617040346.csv'

# load data
dt <- read.csv(here::here('data', file))

# find columns that contain *only* NA and eliminate
d1 <- dt[colSums(is.na(dt))/nrow(dt) < 1]
rm(dt)
...
d2 <- d1 %>%
  ...
  unite('n', colnames(d1)[grep('Discounting.Baseline.N', colnames(d1))], 
        sep = '_', na.rm = TRUE) %>%
  unite('tvalue', c(Discounting.Baseline.t.value, Discounting.Baseline.t.value.1, Discounting.Baseline.tvalue, 
                    Discounting.Baseline.T.value), sep = '', na.rm = TRUE) %>%
  unite('mean', c(Discounting.Baseline.Mean, Discounting.Baseline.M, Discounting.Baseline.mean), sep =  '', na.rm = TRUE)
...
hampton['se'] <- NA; hampton['sd'] <- NA; hampton['Fvalue'] <- NA; hampton['mean'] <- NA; hampton['effect_size_d'] <- NA; 
...
johnson['se'] <- NA; johnson['sd'] <- NA; johnson['Fvalue'] <- NA; johnson['mean'] <- NA; johnson['effect_size_d'] <- NA;
...
dt['se'] <- NA; dt['sd'] <- NA; dt['Fvalue'] <- NA; dt['mean'] <- NA; dt['effect_size_d'] <- NA; dt['condition'] <- NA;
wolfe <- dt; rm(dt)
...
reimers['se'] <- NA; reimers['sd'] <- NA; reimers['Fvalue'] <- NA; reimers['mean'] <- NA; reimers['effect_size_d'] <- NA; reimers['condition'] <- NA; 
...
li['se'] <- NA; li['Fvalue'] <- NA; li['effect_size_d'] <- NA; li['correlation'] <- NA; 
rm(li_mean, li_sd)
...
green['correlation'] <- NA; green['sd'] <- NA
green <- green[which(green$condition != '$10,000'),] # age group comparison not done on higher value
...
whelan['correlation'] <- NA; whelan['sd'] <- NA
...
jimura['correlation'] <- NA; jimura['sd'] <- NA
...
order <- colnames(d2)
order <- order[c(1:10, 22, 11:14, 19, 16, 20, 21, 18, 17)] # put columns in logical order
d2 <- d2[order]
